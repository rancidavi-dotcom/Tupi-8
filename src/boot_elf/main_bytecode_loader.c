 ...
 *
 *   [C] Flatpak / instalação em prefixo:
 *       O binário roda como "tupi" em /app/bin/tupi (ou PREFIX/bin/tupi),
 *       lê scripts de /app/share/tupi/game.tuzip e assets de
 *       /app/share/tupi/assets, sem precisar extrair nada em /tmp.
 *
...
#define TUPI_FLATPAK_SCRIPT_ARCHIVE "/app/share/tupi/game.tuzip"
...
static int tupi_has_suffix(const char *str, const char *suffix) {
    size_t str_len;
    size_t suffix_len;
...
static int tupi_resolve_cli_archive_path(int argc,
                                         char **argv,
    ...
    if (!out || out_size == 0u || argc < 2 || !argv || !argv[1])
        return 0;

    arg = argv[1];
    if (arg[0] == '\0' || !tupi_has_suffix(arg, ".tuzip"))
        return 0;

    if (arg[0] == '/') {
        written = snprintf(out, out_size, "%s", arg);
    } else if (start_cwd && start_cwd[0] != '\0') {
        written = snprintf(out, out_size, "%s/%s", start_cwd, arg);
    } else {
        written = snprintf(out, out_size, "%s", arg);
    }
...
static zip_t *tupi_open_sidecar_zip(const char *cli_archive_path) {
    const char *candidates[4];
    const char *env_path = getenv(TUPI_SCRIPT_ARCHIVE_ENV);
    size_t      i;

    candidates[0] = env_path;
    candidates[1] = cli_archive_path;
    candidates[2] = TUPI_LOCAL_SCRIPT_ARCHIVE;
    candidates[3] = TUPI_DEFAULT_SCRIPT_ARCHIVE;

    for (i = 0; i < 4u; i++) {
        if (!candidates[i] || candidates[i][0] == '\0') continue;
        if (access(candidates[i], R_OK) == 0) {
            zip_t *za = tupi_open_zip_file(candidates[i]);
            if (za) {
                setenv(TUPI_SCRIPT_ARCHIVE_ENV, candidates[i], 1);
                return za;
            }
        }
    }

    if (access(TUPI_FLATPAK_SCRIPT_ARCHIVE, R_OK) == 0) {
        zip_t *za = tupi_open_zip_file(TUPI_FLATPAK_SCRIPT_ARCHIVE);
        if (za) {
            setenv(TUPI_SCRIPT_ARCHIVE_ENV, TUPI_FLATPAK_SCRIPT_ARCHIVE, 1);
            return za;
        }
    }
...
static void tupi_try_set_asset_dir_from_script_archive(const char *script_archive) {
    char archive_dir[PATH_MAX];
    char candidate[PATH_MAX];
    char *slash;
    int   written;

    if (!script_archive || script_archive[0] == '\0')
        return;

    strncpy(archive_dir, script_archive, sizeof(archive_dir) - 1u);
    archive_dir[sizeof(archive_dir) - 1u] = '\0';

    slash = strrchr(archive_dir, '/');
    if (!slash)
        return;
    *slash = '\0';

    written = snprintf(candidate, sizeof(candidate), "%s/assets", archive_dir);
    if (written < 0 || (size_t)written >= sizeof(candidate))
        return;

    if (access(candidate, R_OK) == 0)
        setenv(TUPI_ASSET_DIR_ENV, candidate, 0);
}

static void tupi_configure_default_asset_dir(void) {
    if (getenv(TUPI_ASSET_DIR_ENV)) return;

    tupi_try_set_asset_dir_from_script_archive(getenv(TUPI_SCRIPT_ARCHIVE_ENV));
    if (getenv(TUPI_ASSET_DIR_ENV)) return;
...
int main(int argc, char **argv) {
    ...
    char        cli_archive_path[PATH_MAX];
    TupiArchive archive;
    lua_State  *L;
    long        zip_offset;
    ...
    if (!tupi_resolve_cli_archive_path(argc, argv, start_cwd,
                                       cli_archive_path,
                                       sizeof(cli_archive_path))) {
        cli_archive_path[0] = '\0';
    }

    archive.za = tupi_open_sidecar_zip(cli_archive_path[0] ? cli_archive_path : NULL);
